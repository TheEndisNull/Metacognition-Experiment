pltAgreeByCond <-
  function(cond, CairoPlt, savePlt) {
...
p1 <- ggplot(testvar <- filter(graphDf, grepl(cond, name) & (group == "sub")) %>%
mutate(agremnt = ''), 
...
p2 <- ggplot(filter(graphDf, grepl(cond, name)) %>%
  filter(group == "advsr"), 
  ...
  geom_hline(yintercept = .60,
        ...
        labels = c(
          ...
          'L_LL' = 'Un-Calibrated\n Low Confidence'
            ...
            labs(title = 'Advisor Accuracy with Participant Agreement',
