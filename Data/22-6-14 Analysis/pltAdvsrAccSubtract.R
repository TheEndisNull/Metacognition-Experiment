...
one <- filter(pltDf, group=='hiCal_hiConf') %>%
mutate(winRate = winRate - .8)
two <- filter(pltDf, group=='hiCal_loConf') %>%
mutate(winRate = winRate - .6)

three <- filter(pltDf, group=='loCal_hiConf') %>%
mutate(winRate = winRate - .7)
four <- filter(pltDf, group=='loCal_loConf') %>%
mutate(winRate = winRate - .7)
